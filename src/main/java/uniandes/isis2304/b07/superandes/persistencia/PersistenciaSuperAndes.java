	...
	public String consultarMejoresYPeoresProductosProveedoresSemanaASemanaDeUnAnio(int anio )
	{
		...
		{						
			for(int i = 1 ; i<54;i++)
			{
				...
				{
					resp +="\n producto m�s vendido de la semana "+i+" del a�o 20"+anio;
					Object[] objects = listaVenta.get(0);
					resp += "\n     unidades vendidas             codigo producto";
					resp +="\n                "+objects[0]+"                                "+objects[1];
					resp += "\n producto menos vendido de la semana "+i+"del a�o 20"+anio;
					objects = listaVenta.get(listaVenta.size()-1);
					resp += "\n     unidades vendidas             codigo producto";
					resp += "\n               "+objects[0]+"                                   "+objects[1];
				}  
				else {
					resp += "\n no hubier�n ventas en la semana "+i+" del a�o 20"+anio;
				}
				if(!listaProve.isEmpty())
				{
					resp +="\n proveedor m�s solicitado de la semana "+i+" del a�o 20"+anio;
					Object[] objects = listaProve.get(0);
					resp += "\n     numero de pedidos             nit del proveedor";
					resp += "\n               "+objects[0]+"	         "+objects[1];
					resp += "\n proveedor menos solicitado  de la semana "+i+" del a�o 20"+anio;
					objects = listaVenta.get(listaProve.size()-1); 
					resp += "\n     numero de pedidos             nit del proveedor";
					resp += "\n          "+objects[0]+"       	     "+objects[1];
				}
				else {
					resp += "\n no hubier�n compras aprovedores en la semana "+i+" del a�o 20"+anio;
				}
				resp+="\n ";
			}
		} 
